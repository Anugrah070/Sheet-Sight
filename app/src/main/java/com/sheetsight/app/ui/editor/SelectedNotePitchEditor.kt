...
internal object SelectedNotePitchEditor {
    ...
    ): SelectedNotePitchEdit = edit(
        scoreId = scoreId,
        sourceBytes = sourceBytes,
        noteIdentity = noteIdentity,
        diatonicOffset = if (direction == NaturalNoteDirection.UP) 1 else -1
    )
    ...
    fun edit(
        ...
        if (diatonicOffset == 0) {
            throw SelectedNotePitchEditException("The requested pitch movement is empty.")
        }
        ...
        val absoluteNaturalIndex = currentOctave * NATURAL_STEPS.size + currentIndex + diatonicOffset
        val targetOctave = Math.floorDiv(absoluteNaturalIndex, NATURAL_STEPS.size)
        val targetIndex = Math.floorMod(absoluteNaturalIndex, NATURAL_STEPS.size)
